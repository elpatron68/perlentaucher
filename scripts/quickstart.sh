#!/bin/bash
# Quickstart-Script für Perlentaucher (Linux)
...
echo "=========================================="
echo "Perlentaucher - Quickstart (Linux)"
echo "=========================================="
echo ""
...
show_python_instructions() {
    ...
    # Versuche Package Manager zu erkennen
    if command -v apt-get &> /dev/null; then
        ...
        echo "  Installiere Python 3.7+ über deinen Package Manager"
...
check_pip() {
    if command -v pip3 &> /dev/null || python3 -m pip --version &> /dev/null; then
        ...
        if python3 -m ensurepip --upgrade 2>/dev/null; then
            ...
            echo "  oder: curl https://bootstrap.pypa.io/get-pip.py -o get-pip.py"
...
if [ ! -f "$CONFIG_FILE" ]; then
    ...
    echo "Führe zuerst das Quickstart-Script aus: ./scripts/quickstart.sh"
    exit 1
fi
